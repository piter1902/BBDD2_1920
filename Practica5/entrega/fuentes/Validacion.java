...
public class Validacion {
	...
	public static void main(String[] args) throws IOException {
		...
		// para cada cliente del fichero, leer caracteristicas xi y clasificacion real
		// yi
		// calcular clasificacion predicha segun thetas: yiPredicha = 1/(1 +
		// exp(-xi*thetas)>0.5
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				String clase = elems[1];

				double prod = thetas[0]; // pq la primera es un 1 siempre
				for (int i = 2; i < elems.length; i++) {
					prod += Double.parseDouble(elems[i]) * thetas[i - 1];
				}
				...
				if (clase.equals("A")) {
					...
					if (g < 0.5)
						...
						bfp++; // modelo predice B y es A --> bfp
				} else {
					// letra es B
					if (g < 0.5)
						...
						bpos++; // modelo predice B y es B --> bpos
				}
			}
			scanner.close();
			...
			System.err.println("Error: fichero no existe " + datos);
		}
